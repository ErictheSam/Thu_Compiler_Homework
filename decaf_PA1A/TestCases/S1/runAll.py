...
import os
import subprocess #操作system的bash
import sys
...
def main():
    decaf_jar = os.path.join('..', '..', 'result', 'decaf.jar')#合在一起
    names = sys.argv[1:]#从第一个到最后一个()
    if not names:
        names = sorted(os.listdir('.'))#如果没有指定的话
    for name in names:
        bname,ext = os.path.splitext(name)#按照每个的最后一串给split出来
        if ext != '.decaf':
            continue
        # Run the test case, redirecting stdout/stderr to output/bname.result
        subprocess.call(['java', '-jar', decaf_jar, '-l', '0', name],#开始
                stdout=open(os.path.join('output', bname + '.result'), 'w'),#一串
