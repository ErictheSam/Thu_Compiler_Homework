...
public abstract class Tree {
     ...
     */
    public static final int IMPORT = TOPLEVEL + 1;//归一的是2

    /**
     * Class definitions, of type ClassDef.
     */
    public static final int CLASSDEF = IMPORT + 1;//cclassdef是3

    /**
     * Method definitions, of type MethodDef.
     */
    public static final int METHODDEF = CLASSDEF + 1;//4

    /**
     * Variable definitions, of type VarDef.
     */
    public static final int VARDEF = METHODDEF + 1;//5号

    /**
     * The no-op statement ";", of type Skip
     */
    public static final int SKIP = VARDEF + 1;//6号

    /**
     * Blocks, of type Block.
     */
    public static final int BLOCK = SKIP + 1;//7号

    /**
     * Do-while loops, of type DoLoop.
     */
    public static final int DOLOOP = BLOCK + 1;//8

    /**
     * While-loops, of type WhileLoop.
     */
    public static final int WHILELOOP = DOLOOP + 1;//9

    /**
     * For-loops, of type ForLoop.
     */
    public static final int FORLOOP = WHILELOOP + 1;//10
    ...
    public static final int SUBSTMT = IF + 1;
    
    public static final int GUARDSTMT = SUBSTMT + 1;
    /**
     * Expression statements, of type Exec.
     */
    public static final int EXEC = GUARDSTMT + 1;
    ...
    public static final int DEFT = INDEXED + 1;
    
    public static final int TRAVAIL = DEFT + 1;
    
    public static final int ACCESS = TRAVAIL + 1; 

    /**
     * Selections, of type Select.
     */
    public static final int SELECT = ACCESS + 1;
    ...
    public static final int VARIDENT = IDENT + 1;
    
   // public static final int VARASSIGN = IN + 1;
    
    public static final int SCOPY = VARIDENT + 1;
    /**
     * Literals, of type Literal.
     */
    public static final int LITERAL = SCOPY + 1;
    /**
     * ArrayConstants, of type arrayConstant
     */
    public static final int ARRAYCONST = LITERAL + 1;
    
    public static final int BINARRAYCONST = ARRAYCONST + 1;
    
    public static final int TIMEARRAYCONST = BINARRAYCONST + 1;

    /**
     * Basic type identifiers, of type TypeIdent.
     */
    public static final int TYPEIDENT = TIMEARRAYCONST + 1;
    ...
    public static final int GUARD = OR + 1;
    public static final int AND = GUARD + 1;
 ...
 //   public static final int VAR = VOID + 1;
     ...
     */
    public Tree(int tag, Location loc) {//为tree标识tag和location
        super();
        this.tag = tag;//是那个tree
        this.loc = loc;
    }

	public Location getLocation() {//说出location
		return loc;
	}
    ...
    /**
      ...
      */
    public void accept(Visitor v) {//说出v
        v.visitTree(this);
    }

	public abstract void printTo(IndentPrintWriter pw);

    public static class TopLevel extends Tree {//从toplevel延伸tree

		public List<ClassDef> classes;
		
		public TopLevel(List<ClassDef> classes, Location loc) {//从location出来
			super(TOPLEVEL, loc);
    ...
    public static class ClassDef extends Tree {//在上面
    	
    	public String name;
    	public String parent;
    	public List<Tree> fields;//一串fields就是类成员/方法
    	...
    	@Override
    	public void printTo(IndentPrintWriter pw) {//未完待续
    		if (sealed) {
    		...
    		pw.incIndent();
    		for (Tree f : fields) {
    			f.printTo(pw);//每一部分都开始print
    		}
    		pw.decIndent();//print完了再开始干活
    	}
   }

    public static class MethodDef extends Tree {//方法
        ...
        public MethodDef(boolean statik, String name, TypeLiteral returnType,//是否是static的
        		List<VarDef> formals, Block body, Location loc) {
            super(METHODDEF, loc);
    		this.statik = statik;
    		this.name = name;//方法名称
    		this.returnType = returnType;//最终的type
    	...
    	@Override
    	public void printTo(IndentPrintWriter pw) {//print
    		if (statik) {
    			pw.print("static ");
    		}
    		pw.print("func " + name + " ");
    ...
    public static class VarDef extends Tree {//出去一棵树
    	
    	public boolean usage;
    	public String name;
    	public TypeLiteral type;

        public VarDef(boolean usage,String name, TypeLiteral type, Location loc) {//如果有name,type,loc这些东西
            super(VARDEF, loc);
            this.usage = usage;
    		this.name = name;
    		this.type = type;
    	...
    	@Override
    	public void printTo(IndentPrintWriter pw) {//define一个var
    		if(usage)
    			pw.print("varbind "+ name + " ");//按照不同的
    		else
    ...
    /**
      * A no-op statement ";".
      */
    public static class Skip extends Tree {//skip
    ...
    public static class Block extends Tree {//block

    	public List<Tree> block;
 
        public Block(List<Tree> block, Location loc) {//为什么会有block这个
            super(BLOCK, loc);//相关的
    		this.block = block;
        }
    	...
    	@Override
    	public void printTo(IndentPrintWriter pw) {
    		pw.println("stmtblock");//stmtblock
//    		pw.println(block.size());
    		pw.incIndent();
    		for (Tree s : block) {
    			if(s != null)
    				s.printTo(pw);
    ...
    public static class ForeachLoop extends Tree {
    	...
    	public ForeachLoop(VarDef var, Expr rexpr, Expr condexpr, Tree loopbody,Location loc) {
    ...
    public static class SubStmt extends Tree{//每一个节点
    	
    	public Expr lexpr;//可以做一部分的事情
    	public Tree rexpr;//可以print一棵语法树
    	
    	public SubStmt(Expr lexpr, Tree rexpr, Location loc) {
    		super(SUBSTMT,loc);
    		this.lexpr = lexpr;
    		this.rexpr = rexpr;
    	}
    ...
    public static class GuardStmt extends Tree {
    	...
    	public List<SubStmt> works;
    	
    	public GuardStmt(List<SubStmt> works,Location loc) {
    		super(GUARDSTMT,loc);
    		this.works = works;
    	}
    ...
    /**
      * A binary operation.
      */
    public static class Binary extends Expr {//算东西的
    ...
    /**
     * Access one part of a arrayconst
     */
    public static class Access extends LValue {
    	
    	public Expr array;
    	public Expr lindex;
    	public Expr rindex;
    	
    	public Access(Expr array, Expr lindex, Expr rindex,Location loc) {
    		super(ACCESS,loc);
    		this.array = array;
    		this.lindex = lindex;
    		this.rindex = rindex;
    ...
    /**
      * An identifier
      */
    public static class Ident extends LValue {//ident里面的print
    ...
    public static class ArrayConst extends Expr {
    	...
    	public List<Expr> contents;
    	
    	public ArrayConst(List<Expr> contents, Location loc) {
    		super(ARRAYCONST,loc);
    		this.contents=contents;
    	}
    	
    	public void printTo(IndentPrintWriter pw) {//需要在用的时候重写一下这个pw
    ...
    public static class BinArrayConst extends Expr {
    	public Expr lexpr;
    	public Expr rexpr;
    	
    	public BinArrayConst(Expr lexpr, Expr rexpr, Location loc) {
    		super(BINARRAYCONST, loc);
    		this.lexpr = lexpr;
    		this.rexpr = rexpr;
    	}
    ...
    public static class TimeArrayConst extends Expr {
    	public Expr lexpr;
    	public Expr rexpr;
    	
    	public TimeArrayConst(Expr lexpr, Expr rexpr, Location loc) {
    		super(TIMEARRAYCONST,loc);
    		this.lexpr = lexpr;
    		this.rexpr = rexpr;
    	}
     ...
     */
    public static class DefaultConst extends Expr {
    	public Expr array;
    	public Expr rank;
    	public Expr rexpr;
    	
    	public DefaultConst(Expr array, Expr rank, Expr rexpr, Location loc) {
    		super(DEFT,loc);
    		this.array = array;
    		this.rank = rank;
    		this.rexpr = rexpr;
    ...
    public static class Travail extends Expr {
    	public Expr lexpr;
    	public String ident;
    	public Expr rexpr;
    	public Expr condexpr;
    	
    	public Travail(Expr lexpr, String ident, Expr rexpr, Expr condexpr,Location loc) {
    		super(TRAVAIL,loc);
    		this.lexpr = lexpr;
    		this.rexpr = rexpr;
    		this.condexpr = condexpr;
