...
public final class Driver {
	...
	private void compile() {
		...
		Tree.TopLevel tree = parser.parseFile();//先parse一遍,然后check,再然后
		checkPoint();
		if (option.getLevel() == Option.Level.LEVEL0) {//如果没有的话
		...
		checkPoint();
		TypeCheck.checkType(tree);//最后一次把tree中间的东西给检查一遍
		checkPoint();
		if (option.getLevel() == Option.Level.LEVEL1) {//
			IndentPrintWriter pw = new IndentPrintWriter(option.getOutput(), 4);
			tree.globalScope.printTo(pw);//直接
			pw.close();
			return;
		}
	}
