...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitForeachLoop(Tree.ForeachLoop foreachLoop) {
		...
		/*foreachLoop.ownBlock.associatedScope = new LocalScope(froeachLoop.ownBlock);//对于LocalScope的话,先弹入是比较好的
		table.open(foreachLoop.ownBlock.associatedScope);//得要重写block部分
		foreachLoop.left.accept(this);//先define一个东西进去
	...
	@Override
	public void visitIdent(Tree.Ident ident) {//这里是否产生
		if (ident.owner == null) {
			...
			if (v == null) {
				...
				ident.type = var.getType();//vat.getType()
				ident.symbol = var;
				if (var.isLocalVar()) {
