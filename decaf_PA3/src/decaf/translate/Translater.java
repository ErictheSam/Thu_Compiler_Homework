...
public class Translater {
	...
	public static Translater translate(Tree.TopLevel tree) {
		...
		tp1.visitTopLevel(tree);///visitor设计模式
	...
	public void append(Tac tac) {
		if (currentFuncty.head == null) {
			...
			tac.prev = currentFuncty.tail;//前一个是tail
	...
	public Temp genDirectCall(Label func, Type retType) {//wir sind das 
		Temp dst;
		if (retType.equal(BaseType.VOID)) {
			dst = null;
		} else {
	...
	public void genBeqz(Temp cond, Label dst) {//是否是gen的
	...
	public Temp genLoad(Temp base, int offset) {//那一段
		Temp dst = Temp.createTempI4();
		append(Tac.genLoad(dst, base, Temp.createConstTemp(offset)));//好的
		return dst;
	}
	...
	public void genCheckArrayIndex(Temp array, Temp index) {
		Temp length = genLoad(array, -OffsetCounter.WORD_SIZE);//先把Array-4给load进来
		Temp cond = genLes(index, length);//是否index小于length
	...
	public void getCheckDefaultArrayIndex(Temp array, Temp index, Temp ret, Label err) {
		Temp length = genLoad(array, -OffsetCounter.WORD_SIZE);
		Temp cond = genLes(index,length);
		genBeqz(cond, err);
		cond = genLeq(genLoadImm4(0),index);
		genBeqz(cond, err);
	...
	public void genCheckNewWithArraySize(Temp size) {
		Label exit = Label.createLabel();
		Temp cond = genLes(size, genLoadImm4(0));
		genBeqz(cond, exit);
		Temp msg = genLoadStrConst(RuntimeError.NEGATIVE_GEN_ARR);
		genParm(msg);
		genIntrinsicCall(Intrinsic.PRINT_STRING);
		genIntrinsicCall(Intrinsic.HALT);
		genMark(exit);
	...
	public Temp genNewWithArray(ClassScope tip, Temp sto1, Temp length) {
		genCheckNewWithArraySize(length);
		Temp unit;
		Temp size;
		unit = genLoadImm4(OffsetCounter.WORD_SIZE);
		size = genAdd(unit, genMul(unit, length));
		genParm(size);
		Temp obj = genIntrinsicCall(Intrinsic.ALLOCATE);
		genStore(length, obj, 0);
		Label loop = Label.createLabel();
		Label exit = Label.createLabel();
		Temp zero = genLoadImm4(0);
		append(Tac.genAdd(obj, obj, size));//在这里先加上size
		genMark(loop);
		append(Tac.genSub(size, size, unit));
		genBeqz(size, exit);
		append(Tac.genSub(obj, obj, unit));
		if( tip != null ) {
			genScopy(tip,zero,sto1);
			genStore(zero,obj,0);
		}
		else
			genStore(sto1, obj, 0);
	...
	public void genScopy(ClassScope scp, Temp tmp1, Temp tmp2) {
		int stoy = 4;
		for(Symbol v : scp.symbols.values()) {
			if(v.isVariable()) {
				stoy += 4;
			}
		}
		Temp size = genLoadImm4(stoy);
		genParm(size);
		Temp obj = genIntrinsicCall(Intrinsic.ALLOCATE);
		for( Symbol v : scp.symbols.values() ) {
			if(v.isVariable()) {
				Variable j = (Variable) v;
				Temp newTemp = genLoad(tmp2,j.getOffset());
				genStore(newTemp,obj,j.getOffset());
			}
		}
		genStore(genLoadVTable(scp.getOwner().getVtable()),obj,0);
		genAssign(tmp1,obj);
	}
