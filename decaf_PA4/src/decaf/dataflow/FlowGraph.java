...
public class FlowGraph implements Iterable<BasicBlock> {
    ...
    public Map<Temp,Set<BasicBlock>> Defed;//
    ...
    public FlowGraph(Functy func) {
        ...
        Defed = new TreeMap<Temp,Set<BasicBlock>>(Temp.ID_COMPARATOR);
        analyzeDUChain();
        for (BasicBlock bb : bbs) {
        	bb.analyzeDUChain();
        }
    ...
    public void analyzeDUChain() {
    	for (BasicBlock bb : bbs) {
    		bb.computeDUDefAndLiveUse(Defed);
            bb.computeDEFED(Defed);//先把DU链上的那堆东西给搞清楚!
        }
             ...
             for (BasicBlock bb : bbs) {
                 for (int i = 0; i < 2; i++) {
                     if (bb.next[i] >= 0) { // Not RETURN
                         bb.liveDUOut.addAll(bbs.get(bb.next[i]).liveDUIn);//liveOut全部给加进去!
                     }
                 }
                 bb.liveDUOut.removeAll(bb.DUdef);
                 if (bb.liveDUIn.addAll(bb.liveDUOut))
                     changed = true;
                 for (int i = 0; i < 2; i++) {
                     if (bb.next[i] >= 0) { // Not RETURN
                         bb.liveDUOut.addAll(bbs.get(bb.next[i]).liveDUIn);
                     }
                 }
             }
         } while (changed);
        for(BasicBlock bb: bbs) {
        	bb.analyzeDUChain();//把所有的部分都给gather到最终的地方去
        }
    }
